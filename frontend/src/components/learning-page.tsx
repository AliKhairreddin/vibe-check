...
export function LearningPage() {
  ...
  return <div className="mx-auto grid w-full min-w-0 max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {value?.available ? <>
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="size-5" />Version history & changes</CardTitle><CardDescription>Saved versions include the guideline text and evaluated decisions. Any text changes are shown below.</CardDescription></CardHeader>
        <CardContent className="grid gap-3">{value.versions.map(version => <details key={version.version} open={restore?.version === version.version || requestedVersion === version.version} className="rounded-lg border p-4">
          ...
          <p className="mt-3 text-sm leading-6">{version.reason}</p><p className="mt-1 text-xs text-muted-foreground">{version.actor === 'automatic' ? 'Saved automatically by AdChecked' : version.actor === 'system' ? 'Saved by AdChecked' : `Saved by ${version.actor}`}</p>
        ...
        {!value.versions.length ? <p className="text-sm text-muted-foreground">Saved versions will appear here after feedback is processed.</p> : null}
