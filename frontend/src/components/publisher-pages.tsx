...
import { SharedLinksPanel } from './share-controls';
...
export function PublisherUploadsPage() {
  ...
  return <ClientPortalFrame><div className="mx-auto grid max-w-6xl gap-6"><div><p className="text-sm text-muted-foreground">{advertiser} / {session.publisher_name}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Uploads &amp; progress</h1><p className="mt-2 text-sm text-muted-foreground">Upload creatives, track their checks, and see feedback from {advertiser}.</p></div>
    ...
    <section className="grid gap-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Your submissions</h2><p className="mt-1 text-sm text-muted-foreground">Track upload progress here. Open Review queue to release batches and send email.</p><Link to="/client/reviews" className={buttonVariants({ size: 'sm', variant: 'outline', className: 'mt-3' })}>Review batches</Link></div></div>
    {query.isLoading ? <p>Loading submissions…</p> : query.error ? <p role="alert" className="text-destructive">{query.error.message}</p> : !query.data?.length ? <div className="rounded-xl border border-dashed p-10 text-center text-sm text-muted-foreground">Your submitted creatives will appear here.</div> : query.data.map(row => <div key={row.jobId} className="flex flex-wrap items-center gap-3 rounded-xl border bg-card p-4"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{row.fileName}</p><p className="mt-1 text-xs text-muted-foreground">{row.message} {row.status !== 'complete' && row.status !== 'failed' ? `· ${row.progress}%` : ''}</p></div><Badge variant="outline">{row.status}</Badge>{row.status === 'complete' ? <><Badge variant={row.released ? 'secondary' : 'outline'}>{row.released ? 'Released' : 'Private'}</Badge><Link to="/client/$clientId/reviews/$jobId" params={{ clientId, jobId: row.jobId }} className={buttonVariants({ size: 'sm', variant: 'outline' })}>View review</Link></> : null}{!row.released && ['complete', 'failed'].includes(row.status) ? <DeleteSubmissionButton jobId={row.jobId} fileName={row.fileName} clientId={clientId} /> : null}</div>)}
